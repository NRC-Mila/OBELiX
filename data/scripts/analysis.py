...
def best_permutation_dist(df):
    for i, cid_row in enumerate(df.iterrows()):
        ...
        for j in permutations:
            ...
            df.loc[cid, ["alpha", "beta", "gamma"]] = df_cp[
                ["alpha", "beta", "gamma"]
            ].to_numpy()[j]
            if distance(df.iloc[: i + 1]).max() < best_distance.max():
                best_distance = distance(df.iloc[: i + 1]).max()
                best_permutation = j
        df.loc[cid, ["a", "b", "c"]] = df_cp[["a", "b", "c"]].to_numpy()[
            best_permutation
        ]
        df.loc[cid, ["alpha", "beta", "gamma"]] = df_cp[
            ["alpha", "beta", "gamma"]
        ].to_numpy()[best_permutation]
    return distance(df)
...
def plot_similar(df, subset, palette=["red", "blue"]):
    ...
    print(
        "Number of repeated compositions: ",
        len(repeated_comps),
        "/",
        len(duplicated_df),
    )
    ...
    for i, dat in enumerate(repeated_comps.iterrows()):
        ...
        diff = (
            np.log10(same_comp["Ionic conductivity (S cm-1)"]).max()
            - np.log10(same_comp["Ionic conductivity (S cm-1)"]).min()
        )
        ...
        scatter_points.append(
            [[i] * len(df_list[i]), same_comp["Ionic conductivity (S cm-1)"].to_list()]
        )
        ...
        stds_alt.append(
            np.log10(same_comp["Ionic conductivity (S cm-1)"])
            - np.log10(same_comp["Ionic conductivity (S cm-1)"]).mean()
        )
        maes_alt.append(
            np.log10(same_comp["Ionic conductivity (S cm-1)"])
            - np.log10(same_comp["Ionic conductivity (S cm-1)"]).median()
        )

    print("Average log standard deviation: ", np.mean(stds))

    print(
        "Average log standard deviation (RMSE like): ",
        np.sqrt(np.mean(np.concatenate(stds_alt) ** 2)),
    )
    print(
        "Average log difference with mean (MAE like): ",
        np.mean(abs(np.concatenate(maes_alt))),
    )

    plt.figure()
    # plt.boxplot(np.concatenate(stds_alt))
    diff_df = pd.DataFrame(
        np.concatenate(stds_alt),
        columns=["Difference with mean log$_{10}$(Ionic Conductivity)"],
    )
    sns.violinplot(
        data=diff_df["Difference with mean log$_{10}$(Ionic Conductivity)"],
    ...
    test_mask = np.array(
        [row["in_test"] for i in idx for _, row in df_list[i].iterrows()]
    )
    ...
    s = ax.scatter(
        scatter_points[0, :][~test_mask],
        scatter_points[1, :][~test_mask],
        s=30,
        c=colors[~test_mask],
        cmap=cm,
        alpha=0.8,
    ...
    cbar = plt.colorbar(s)
    cbar.set_label(
        "Max \% difference between lattice parameters", rotation=270, labelpad=15
    )
    fig.tight_layout()
    ...
    diffs = [
        abs(d)
        for i in idx
        for d in np.log10(df_list[i]["Ionic conductivity (S cm-1)"])
        - np.log10(df_list[i]["Ionic conductivity (S cm-1)"]).mean()
    ]
...
def get_cell_volume(a, b, c, alpha, beta, gamma):
    return (
        ...
        * (
            1
            - np.cos(np.deg2rad(alpha)) ** 2
            - np.cos(np.deg2rad(beta)) ** 2
            - np.cos(np.deg2rad(gamma)) ** 2
            + 2
...
def density_of_carriers(data):
    ...
    s = plt.scatter(
        np.log10(Lidensity),
        np.log10(space),
        c=data["Space group number"].to_numpy() - 1,
        alpha=0.8,
        s=10,
        cmap=spg_cmp,
        vmin=0,
...
def distributions(data):
    ...
    ax = sns.displot(
        data=data,
        x="Ionic conductivity (S cm-1)",
        hue="CIF",
        kde=True,
        multiple="stack",
        row="in_test",
        log_scale=True,
        hue_order=["No Match", "Close Match", "Match"],
...
def periodic_table_map(data):
    ...
    from pymatviz import (ptable_heatmap, ptable_heatmap_ratio,
    ...
    ptable_heatmap(
        occurences / 5.99,
        colormap="ocean_r",
        cbar_kwargs={"norm": colors.LogNorm(vmin=0, vmax=100)},
        nan_color="w",
        cbar_title="Prevalence in dataset (\%)",
    )

    plt.savefig("periodic_table.svg")

    test_occurences = (
        data["Composition"][data["in_test"]].apply(get_number_of_elements).sum()
    )
    train_occurences = (
        data["Composition"][~data["in_test"]].apply(get_number_of_elements).sum()
    )
    ...
    occurences = pd.concat(
        [
            test_occurences / sum(data["in_test"]),
            train_occurences / sum(~data["in_test"]),
        ],
        axis=1,
    )
...
def print_stats(data):
    ...
    mae_train = np.abs(
        np.log10(data["Ionic conductivity (S cm-1)"][~data["in_test"]]) - mean_train
    ).mean()
    mae_test = np.abs(
        np.log10(data["Ionic conductivity (S cm-1)"][data["in_test"]]) - mean_test
    ).mean()
    ...
    print(
        "Train avg on test: ",
        np.abs(
            mean_train - np.log10(data["Ionic conductivity (S cm-1)"][data["in_test"]])
        ).mean(),
    )
...
def manual_pie_chart(data):
    ...
    spg = [
        [(spg[i][j] if count[i][j] / x.sum() > 0.01 else "") for j in id_orders[i]]
        for i in id_order
    ]
