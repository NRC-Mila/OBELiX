...
def read_options():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Threshold for ionic conductivity. Default is None.",
    )
    parser.add_argument(
        "-d",
        "--del",
        ...
        action="store_true",
        default=False,
        help="Wether to delete close duplicates. Default is False.",
    )
...
def get_paper_from_laskowski(
    material, cond, laskowski_data, doi_lookup_table, return_idx=False
):
    paper_dois = []
    potential_matches = []
    for j, paper_material in enumerate(laskowski_data[:, 0]):
        if (
            ...
            or is_same_formula(material, paper_material)
            or paper_material
            + laskowski_data[min(j + 1, laskowski_data.shape[0] - 1), 0]
            == material
            or paper_material
            + laskowski_data[min(j + 2, laskowski_data.shape[0] - 1), 0]
            == material
        ):
            ...
            if paper_doi == "":
                if (
                    ...
                    + laskowski_data[min(j + 1, laskowski_data.shape[0] - 1), 0]
                    == material
                    and laskowski_data[j + 1, -1] != ""
                ):
                    paper_doi = laskowski_data[j + 1, -1]
                elif paper_material + laskowski_data[
                    min(j + 2, laskowski_data.shape[0] - 1), 0
                ] == material and (
                    laskowski_data[j + 1, -1] != "" or laskowski_data[j + 2, -1] != ""
                ):
            ...
            if cond == cond_mat:
                if paper_doi == "Missing paper info (Laskowski)":
                    print(
                        "WARNING: Found material but paper info is missing:", material
                    )
                ...
                if return_idx:
                    ...
                    paper_dois.extend(
                        [
                            doi_lookup_table[doi]
                            for doi in re.findall(r"[0-9]+", paper_doi)
                        ]
                    )
            else:
                if paper_doi == "Missing paper info (Laskowski)":
                    ...
                    if return_idx:
                        potential_matches.append(
                            (re.findall(r"[0-9]+", paper_doi), cond_mat)
                        )
                    else:
                        potential_matches.append(
                            (
        ...
        if material == liion_material or is_same_formula(material, liion_material):
            if return_idx:
                ...
                if cond == float(liion_data[j, 4]):
                    ...
                    potential_matches.append(
                        ([liion_data[str(j), 2]], float(liion_data[j, 4]))
                    )
...
def get_paper_from_hand(material, cond, hand_data):
    ...
    for j, data in enumerate(hand_data[:, [0, 1]]):
        ...
        if (
            material == hand_material or is_same_formula(material, hand_material)
        ) and float_cond == float_hand_cond:
...
def add_paper_info(database):
    ...
    doi_lookup_table = dict(
        np.genfromtxt(doi_lookup_table_file, delimiter=",", dtype=str, skip_header=1)
    )
    ...
    new_homin_data = np.concatenate(
        (homin_data, np.ones((homin_data.shape[0], 1))), axis=1
    )
    ...
    for i, mat_info in enumerate(homin_data[:, [0, 3]]):
        ...
        paper_info_laskowski, potential_matches_laskowski = get_paper_from_laskowski(
            material, cond, laskowski_data, doi_lookup_table
        )
        paper_info_LiIon, potential_matches_LiIon = get_paper_from_liion(
            material, cond, liion_data
        )
        ...
        if len(paper_info) == 0:
            ...
            for doi, match_cond in potential_matches:
                ...
                if abs(float_match_cond - float_cond) <= abs(
                    float_best_match_cond - float_cond
                ):
            ...
            if len(paper_info) == 0:
                ...
                print(
                    ...
                    ",".join(
                        [
                            '=HYPERLINK("https://doi.org/' + p.replace("*", "") + '")'
                            for p in paper_info
                        ]
                    ),
                    file=close_matches,
                ...
                for doi, match_cond in potential_matches:
                    print(
                        match_cond,
                        ",".join(
                            ['=HYPERLINK("https://doi.org/' + p + '")' for p in doi]
                        ),
                        file=close_matches,
                        sep=",",
                    )
    ...
    if not_found_exactly_count > 0:
        print(
            "WARNING: %d materials were not found exactly in the databases"
            % (not_found_exactly_count)
        )
    if not_found_count > 0:
        print(
            "WARNING: %d materials were not found in the databases" % (not_found_count)
        )

    new_homin_data = pd.DataFrame(
        new_homin_data, columns=database.columns.tolist() + ["DOI"]
    )
...
def remove_close_duplicates(df, tol=1e-4):
    ...
    def is_close(val):
        matches = df.index[
            (df["Composition"].apply(is_same_formula, args=(val["Composition"],)))
            & (abs(df[cols] - val[cols]) / val[cols] < tol).all(axis=1)
        ]
...
def remove_exact_duplicates(df, list_columns=None, index=False):
    ...
    def list_duplicates(val):
        if index:
            ...
            if list_columns is not None:
                return df[
                    (df[list_columns] == val[list_columns]).all(axis=1)
                ].index.tolist()
            else:
...
def main(args):
    ...
    final["Ionic conductivity (S cm-1)"] = final["Ionic conductivity (S cm-1)"].apply(
        replace_text_IC
    )

    if args.thresh is not None:
        final.loc[
            final["Ionic conductivity (S cm-1)"] < args.thresh,
            "Ionic conductivity (S cm-1)",
        ] = args.thresh

    print("Checking for duplicate inputs...")
    final = remove_exact_duplicates(
        final,
        ["Composition", "Space group number", "a", "b", "c", "alpha", "beta", "gamma"],
    )
    print("WARNING: Entries in the left column were removed.")

    print("Checking for duplicate comp+spg and paper...")
    final_close = remove_exact_duplicates(
        final, ["Composition", "Space group number", "DOI"]
    )
    if args.delete:
        final = final_close
        print("WARNING: Entries in the left column were removed.")
        ...
        print(
            "WARNING: Entries should be checked manually. Rerun with -d to remove entries in the left column."
        )
